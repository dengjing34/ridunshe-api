...
class JMSiteEngine
{
    ...
    public function run()
    {
        ...
        $routePathFields = array_map(function ($v) {
            return ucfirst($v);
        }, $this->routePathFields);
        $testControllerClassName = 'Controller_' . join('_', $routePathFields);
        if(JMClassAutoLoader::Load($testControllerClassName) ) {
            $foundControllerClassName = $testControllerClassName;
        } else {
            $tmpFields = $routePathFields;
            array_pop($tmpFields);
        ...
        {
            $lastActionField = end($routePathFields);
        }
